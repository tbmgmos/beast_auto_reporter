...
def fallback_series_key(docx_name: str) -> str:
    ...
    (нет sNN/eNN-меток — например, преролл/рекламный ролик). В этом случае
    meta is None и meta.series недоступен, поэтому в качестве стабильного
    идентификатора (повторяющегося при повторных отправках того же ролика)
    используется само имя файла отчёта — без "отчет_"-префикса и расширения.
    Хвост вида "_2026_03_27_v1"/"_2026_03_27_v1_2" (дата + версия/дубль,
    та же форма, что REPORT_PATTERN отрезает у обычных отчётов) тоже
    отбрасывается — иначе каждая новая версия одного и того же ролика
    получала бы свой ключ, и ручной выбор папки никогда бы не запоминался
    на будущее.
    """
    stem = docx_name
