...
3. Сгруппируй похожие проблемы (щелчки, слюна, шипение, несинхронность, перегруз и т.д.)
...
3. Каждый пункт начинается с "- " (дефис с пробелом и 4 пробела)
...
📍 ОБЫЧНЫЕ ПРОБЛЕМЫ - ПРАВИЛА ГРУППИРОВКИ:
...
Б) Если похожие проблемы встречаются 2-3 раза И это ВАЖНАЯ проблема (несинхронность, отсутствие звука):
   "- На таймкодах 01:19:09:23, 01:19:25:20 и 01:21:34:16 присутствуют реплики, которые выглядят несинхронно с изображением"
   (ВАЖНО: Перечисляй таймкоды через запятую и "и" для последнего)

В) Если проблемы встречаются 4+ раз - ОБОБЩАЙ БЕЗ таймкодов:
...
• Щелчки и слюна:
  - Если щелчков 4+ И слюны 4+ → объединяй:
    "- В фонограмме присутствуют посторонние щёлкающие звуки и яркие звуки слюны"
  - Если только щелчков 4+: "- В фонограмме присутствуют посторонние щёлкающие звуки"
  - Если только слюны 4+: "- В фонограмме присутствуют яркие звуки слюны"
  - Если по 1 разу - каждый с таймкодом отдельно
  - Если 2-3 раза - перечисли таймкоды

• Шипение (высокочастотное/низкочастотное/фоновое) - группируй как одну проблему

• Несинхронность, видимые замены текста - группируй, для 2-3 случаев ПЕРЕЧИСЛИ таймкоды

• Перегруз, пережатые реплики, клиппинг - группируй как "Некоторые реплики звучат пережато"

• Реверберация - группируй как "В нескольких фрагментах изменяется реверберация..."

• Уникальные проблемы (попытки исправления, рекомендации) - с подробным описанием

📝 СТИЛЬ:

✅ ПРАВИЛЬНО:
- Простой, понятный язык
- "В фонограмме присутствует..." (для массовых проблем)
- "На таймкоде XX:XX:XX:XX..." (для единичных)
- "На таймкодах XX, XX и XX..." (для 2-3 важных проблем)
- БЕЗ количества в скобках
- Описательные слова: "слышно", "присутствует", "звучит", "выглядит"

❌ НЕПРАВИЛЬНО:
- "(5 случаев)", "(много)" - НЕ указывай количество!
- Технические термины без объяснения

📚 ПРИМЕРЫ ИЗ РЕАЛЬНЫХ ОТЧЁТОВ:

Массовые проблемы (4+ случаев):
- В фонограмме присутствуют посторонние щёлкающие звуки
- В фонограмме слышно высокочастотное шипение на репликах актёров
- Некоторые реплики звучат пережато. Ощущение, что есть перегруз

Важные проблемы (2-3 случая) - с таймкодами:
- На таймкодах 01:19:09:23, 01:19:25:20 и 01:21:34:16 присутствуют реплики, которые выглядят несинхронно с изображением
- На таймкодах 01:11:42:06 и 01:11:44:22 реплики актёра выглядят несинхронно с изображением

Единичные проблемы:
- Отсутствует звук на заставках "Кинопоиск" и "Студия плюс"
- Звуковая дорожка начинается на таймкоде 01:00:07:12
- На таймкоде 01:18:31:02: посторонний деревянный звук, возможно, слишком яркий звук шага

Сложные проблемы:
- В нескольких фрагментах изменяется реверберация на речи актёров на несоответствующую пространству в кадре (актёры в машине звучат с реверберацией большого помещения)
- Есть ряд не исправленных маркеров (не исправлена маскировка нецензурной лексики на 01:01:45:13, посторонние щелкающие звуки на 01:08:42:22 и 01:08:44:24)
- Есть ряд маркеров, где были попытки исправления, однако проблемы остались (невычищенная слюна)

⚠️ КРИТИЧЕСКИ ВАЖНО:
- Анализируй СМЫСЛ описаний, а не только слова
- "Щелкающий", "цокание", "клик" - это щелчки
- "Шипение", "шуршание", "фоновый шум" - похожие проблемы
- "Несинхронно", "не синхронен", "видна замена" - проблемы синхронизации
- "Пережато", "перегруз", "клиппинг" - проблемы уровней
- Группируй интеллектуально!

Теперь проанализируй ВСЕ проблемы и составь заключение:"""
            
            logger.info("Генерация субъективного заключения через Ollama (FULL LLM с новой логикой)...")
