...
class ExactReportGenerator:
                ...
                if has_audio:
                    ...
                    if duration > 0:
                        ...
                        if not is_frame_aligned(duration, fps):
                            chrono_bg = "FF0000"  # Красный - не кратен кадру
                        ...
                        # Затем проверяем совпадение с референсом
                            ...
                            if durations_match(duration, reference_duration):
                                chrono_bg = "00B050"  # Зеленый - совпадает и кратен кадру
                            else:
                                chrono_bg = "FF0000"  # Красный - не совпадает с референсом
                        else:
                            chrono_bg = "FFC000"  # Оранжевый - нет эталона, но кратен кадру
                    else:
                    ...
                    if key.startswith('audio') and pdf_key and pdf_key in tech_info:
                        ...
                        format_bg = "00B050" if is_complete else "FFFF00"  # Зеленый или желтый
                        ...
                        is_complete = co and (',' in co or 'Stereo' in co or '(' in co)
                        format_bg = "00B050" if is_complete else "FFFF00"
                        self._format_cell(row.cells[6], bg=format_bg, align="left")
    ...
    def _add_me_technical_table(self, doc: Document, tech_info: Dict,
        ...
        try:
                ...
                if has_data:
                    ...
                    if key.startswith('audio') and pdf_key and pdf_key in tech_info:
                        ...
                        if true_peak is not None:
                            ...
                            peak_bg = "00B050" if true_peak <= target_peak else "FF0000"
                    ...
                    if key.startswith('audio'):
                        ...
                        format_text = f"{video_format} {fps_video}fps, TC: {timecode}"
                        row.cells[4].text = format_text
                        self._format_cell(row.cells[4], bg="00B050", align="left")
            ...
            # Форматируем объединенную ячейку
            self._format_cell(cell_merged, bg="FFFFFF", align="center")
            self._format_cell(row.cells[2], bg="00B050")
            self._format_cell(row.cells[3], bg="FF0000")
            self._format_cell(row.cells[4], bg="FFC000")
            ...
            # Строка 8: Текст заключения
            row = table.rows[8]
            cell = row.cells[0]
            for col in range(1, 5):
                cell.merge(row.cells[col])
            
            conclusion_text = f"Техническое заключение:\n{conclusion_tech}\n\nСубъективная оценка:\n{conclusion_subj}"
