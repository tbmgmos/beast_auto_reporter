...
"""
PyInstaller spec for Beast Auto Reporter — Intel (x86_64) standalone .app
Built with Python 3.9 (x86_64) + static ffmpeg for maximum portability.
"""
...
# ── ffmpeg / ffprobe binaries (x86_64 static builds) ─────────────────────
ffmpeg_bin = '/tmp/ffmpeg_x86/ffmpeg'
ffprobe_bin = '/tmp/ffmpeg_x86/ffprobe'
...
sndfile_dylib = os.path.join(sf_dir, '_soundfile_data', 'libsndfile_x86_64.dylib')
...
if os.path.exists(sndfile_dylib):
    ...
    # Also bundle as libsndfile.1.dylib (the canonical name the dylib references itself as)
...
hidden_imports = [
    ...
    'src.audio_analyzer_extended', 'src.spellcheck_service', 'src.secret_store', 'src.spellcheck_review', 'src.yandex_oauth', 'src.yandex_ui.oauth_dialog',
]
